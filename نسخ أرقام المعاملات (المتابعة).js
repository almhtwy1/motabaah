...
(() => {
  ...
  const OUT = new Intl.DateTimeFormat('en-GB',{year:'numeric',month:'2-digit',day:'2-digit',timeZone:'UTC'});
  ...
  // ---------- التشغيل ----------
  function setup(doc){
    if(!doc.querySelector('#MailDataTable') || doc._installed) return;
    doc._installed = true;
  ...
  // مرة أولى + مراقبة إضافات IFRAME فقط (بدون دورات متكررة ثقيلة)
  scan();
