...
(function () {
    ...
    function waitForUserCode(timeoutMs = 5000) {
        return new Promise(resolve => {
            ...
            const t = setInterval(() => {
                ...
                if (code) { clearInterval(t); resolve(code); }
    ...
    (async function init() {
        ...
        if (!ALLOWED_USERS.includes(userCode)) return; // ❌ اطلع إذا اليوزر غير مسموح
        ...
        function applyFilter() {
            var url = location.href.replace(
                'ShowPage?Template=apps/ctsc/templates/Templates/TransferVisualTracking.html&',
                'CTSC?C=GetTrackingHistory&token=&'
            );
